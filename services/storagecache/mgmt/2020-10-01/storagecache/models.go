...
type CacheActiveDirectorySettings struct {
	...
	// DomainNetBios - The Active Directory domain's NetBIOS name.
	DomainNetBios *string `json:"domainNetBios,omitempty"`
	// SmbServerName - The name (NetBIOS) used for the HPC Cache to join the Active Directory domain. Length must not be greater than 15 and chars must be from the [-0-9a-zA-Z] char class.
	SmbServerName *string `json:"smbServerName,omitempty"`
	// DomainJoined - READ-ONLY; This field indicates if the HPC Cache is joined to the Active Directory domain. Possible values include: 'Yes', 'No', 'Error'
	DomainJoined DomainJoinedType `json:"domainJoined,omitempty"`
	// Credentials - Active Directory admin or user credentials used to join the HPC Cache to a domain.
...
func (cads CacheActiveDirectorySettings) MarshalJSON() ([]byte, error) {
	...
	if cads.DomainNetBios != nil {
		objectMap["domainNetBios"] = cads.DomainNetBios
	}
	if cads.SmbServerName != nil {
		objectMap["smbServerName"] = cads.SmbServerName
	}
...
// CacheActiveDirectorySettingsCredentials active Directory admin or user credentials used to join the HPC
// Cache to a domain.
type CacheActiveDirectorySettingsCredentials struct {
	// Username - User name of the Active Directory domain administrator. This value is stored encrypted and not returned on response.
...
type CacheDirectorySettings struct {
	// ActiveDirectory - Specifies the settings for joining the HPC Cache to an Active Directory domain.
	ActiveDirectory *CacheActiveDirectorySettings `json:"activeDirectory,omitempty"`
	// UsernameDownload - Specifies the settings for Extended Groups. Extended Groups allows users to be members of more than 16 groups.
...
// CacheUsernameDownloadSettings settings for username and group download for Extended Groups.
type CacheUsernameDownloadSettings struct {
	// ExtendedGroupsEnabled - This indicates if Extended Groups is enabled.
	ExtendedGroupsEnabled *bool `json:"extendedGroupsEnabled,omitempty"`
	// UsernameSource - This setting determines how the system gets username and group names for clients. Possible values include: 'UsernameSourceAD', 'UsernameSourceLDAP', 'UsernameSourceFile', 'UsernameSourceNone'
	UsernameSource UsernameSource `json:"usernameSource,omitempty"`
	// GroupFileURI - The URI of the file containing the group information (in /etc/group file format). This field must be populated when 'usernameSource' is set to 'File'.
	GroupFileURI *string `json:"groupFileURI,omitempty"`
	// UserFileURI - The URI of the file containing the user information (in /etc/passwd file format). This field must be populated when 'usernameSource' is set to 'File'.
	...
	// EncryptLdapConnection - This indicates if the LDAP connection should be encrypted.
	EncryptLdapConnection *bool `json:"encryptLdapConnection,omitempty"`
	// RequireValidCertificate - Determines if the certificates should be validated by a certificate authority. When true, caCertificateURI must be provided.
	RequireValidCertificate *bool `json:"requireValidCertificate,omitempty"`
	// AutoDownloadCertificate - Determines if the certificate should be automatically downloaded. This applies to 'caCertificateURI' when 'requireValidCertificate' is true, or a self signed certificate otherwise.
	...
	// UsernameDownloaded - READ-ONLY; Indicates if the HPC Cache has performed the username download successfully. Possible values include: 'UsernameDownloadedTypeYes', 'UsernameDownloadedTypeNo', 'UsernameDownloadedTypeError'
...
// NfsAccessRule rule to place restrictions on portions of the NFS namespace being presented to clients.
type NfsAccessRule struct {
	// Scope - Scope applied to this rule. Possible values include: 'Default', 'Network', 'Host'
	Scope NfsAccessRuleScope `json:"scope,omitempty"`
	// Filter - Filter applied to this rule. The filter's format depends on its scope.  'default' scope is reserved for system use. 'network' is in CIDR format (e.g., 10.99.1.0/24) and 'host' is an IP address or fully qualified domain name.
	...
	// SubmountAccess - Allow mounts below the junction.
