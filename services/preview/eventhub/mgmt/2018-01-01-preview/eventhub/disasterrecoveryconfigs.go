...
// DisasterRecoveryConfigsClient is the azure Event Hubs client for managing Event Hubs Cluster, IPFilter Rules and
...
func (client DisasterRecoveryConfigsClient) CreateOrUpdate(ctx context.Context, resourceGroupName string, namespaceName string, alias string, parameters ArmDisasterRecovery) (result DisasterRecoveryConfigsCreateOrUpdateFuture, err error) {
	if tracing.IsEnabled() {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "eventhub.DisasterRecoveryConfigsClient", "CreateOrUpdate", result.Response(), "Failure sending request")
		return
	}
...
// http.Response Body if it receives an error.
func (client DisasterRecoveryConfigsClient) CreateOrUpdateSender(req *http.Request) (future DisasterRecoveryConfigsCreateOrUpdateFuture, err error) {
