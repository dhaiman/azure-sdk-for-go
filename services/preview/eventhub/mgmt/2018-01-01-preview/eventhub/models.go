...
// DisasterRecoveryConfigsCreateOrUpdateFuture an abstraction for monitoring and retrieving the results of
// a long-running operation.
type DisasterRecoveryConfigsCreateOrUpdateFuture struct {
	azure.Future
}

// Result returns the result of the asynchronous operation.
// If the operation has not completed it will return an error.
func (future *DisasterRecoveryConfigsCreateOrUpdateFuture) Result(client DisasterRecoveryConfigsClient) (adr ArmDisasterRecovery, err error) {
	var done bool
	done, err = future.DoneWithContext(context.Background(), client)
	if err != nil {
		err = autorest.NewErrorWithError(err, "eventhub.DisasterRecoveryConfigsCreateOrUpdateFuture", "Result", future.Response(), "Polling failure")
		return
	}
	if !done {
		err = azure.NewAsyncOpIncompleteError("eventhub.DisasterRecoveryConfigsCreateOrUpdateFuture")
		return
	}
	sender := autorest.DecorateSender(client, autorest.DoRetryForStatusCodes(client.RetryAttempts, client.RetryDuration, autorest.StatusCodesForRetry...))
	if adr.Response.Response, err = future.GetResult(sender); err == nil && adr.Response.Response.StatusCode != http.StatusNoContent {
		adr, err = client.CreateOrUpdateResponder(adr.Response.Response)
		if err != nil {
			err = autorest.NewErrorWithError(err, "eventhub.DisasterRecoveryConfigsCreateOrUpdateFuture", "Result", adr.Response.Response, "Failure responding to request")
		}
	}
	return
}
