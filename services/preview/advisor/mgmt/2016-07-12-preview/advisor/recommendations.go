...
// Generatee initiates the recommendation generation or! computation process for a subscription. This operation is
// asynchronous. The generated recommendations are stored in a cache in the Advisor service.
func (client RecommendationsClient) Generatee(ctx context.Context) (result autorest.Response, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/RecommendationsClient.Generatee")
		defer func() {
			sc := -1
	...
	req, err := client.GenerateePreparer(ctx)
	if err != nil {
		err = autorest.NewErrorWithError(err, "advisor.RecommendationsClient", "Generatee", nil, "Failure preparing request")
		return
	}

	resp, err := client.GenerateeSender(req)
	if err != nil {
		result.Response = resp
		err = autorest.NewErrorWithError(err, "advisor.RecommendationsClient", "Generatee", resp, "Failure sending request")
		return
	}

	result, err = client.GenerateeResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "advisor.RecommendationsClient", "Generatee", resp, "Failure responding to request")
	}

	return
}

// GenerateePreparer prepares the Generatee request.
func (client RecommendationsClient) GenerateePreparer(ctx context.Context) (*http.Request, error) {
...
// GenerateeSender sends the Generatee request. The method will close the
// http.Response Body if it receives an error.
func (client RecommendationsClient) GenerateeSender(req *http.Request) (*http.Response, error) {
	sd := autorest.GetSendDecorators(req.Context(), azure.DoRetryWithRegistration(client.Client))
	return autorest.SendWithSender(client, req, sd...)
}

// GenerateeResponder handles the response to the Generatee request. The method always
// closes the http.Response Body.
func (client RecommendationsClient) GenerateeResponder(resp *http.Response) (result autorest.Response, err error) {
