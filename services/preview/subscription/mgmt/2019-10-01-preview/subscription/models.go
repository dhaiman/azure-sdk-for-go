...
type ModernCspSubscriptionCreationParameters struct {
	...
	DisplayName *string `json:"displayName,omitempty"`
	// SkuID - The SKU ID of the Azure plan. Azure plan determines the pricing and service-level agreement of the subscription. Use 0001 for Microsoft Azure Plan and 0002 for Microsoft Azure Plan for DevTest.
...
// PutSubscriptionFuture an abstraction for monitoring and retrieving the results of a long-running
// operation.
type PutSubscriptionFuture struct {
	azure.Future
}

// Result returns the result of the asynchronous operation.
// If the operation has not completed it will return an error.
func (future *PutSubscriptionFuture) Result(client Client) (psr PutSubscriptionResponse, err error) {
	var done bool
	done, err = future.DoneWithContext(context.Background(), client)
	if err != nil {
		err = autorest.NewErrorWithError(err, "subscription.PutSubscriptionFuture", "Result", future.Response(), "Polling failure")
		return
	}
	if !done {
		err = azure.NewAsyncOpIncompleteError("subscription.PutSubscriptionFuture")
		return
	}
	sender := autorest.DecorateSender(client, autorest.DoRetryForStatusCodes(client.RetryAttempts, client.RetryDuration, autorest.StatusCodesForRetry...))
	if psr.Response.Response, err = future.GetResult(sender); err == nil && psr.Response.Response.StatusCode != http.StatusNoContent {
		psr, err = client.PutSubscriptionResponder(psr.Response.Response)
		if err != nil {
			err = autorest.NewErrorWithError(err, "subscription.PutSubscriptionFuture", "Result", psr.Response.Response, "Failure responding to request")
		}
	}
	return
}

// PutSubscriptionRequest the parameters required to create a new subscription.
type PutSubscriptionRequest struct {
	// Properties - Put subscription request properties.
	Properties *PutSubscriptionRequestProperties `json:"properties,omitempty"`
}

// PutSubscriptionRequestProperties put subscription properties.
type PutSubscriptionRequestProperties struct {
	// DisplayName - The friendly name of the subscription.
	DisplayName *string `json:"displayName,omitempty"`
	// SkuID - The SKU ID of the Azure plan. Azure plan determines the pricing and service-level agreement of the subscription. Use 0001 for Microsoft Azure Plan and 0002 for Microsoft Azure Plan for DevTest.
	SkuID *string `json:"skuId,omitempty"`
	// BillingScope - Determines customerLed / fieldLed (/billingAccounts/{billingAccountId}/billingProfiles/{billingProfileId}/invoiceSections/{invoiceSectionId}), partnerLed (/billingAccounts/{billingAccountId}/customers/{customerId}).
	BillingScope *string `json:"billingScope,omitempty"`
	// InitialOwnerPrincipalID - Initial owner principal Id.
	InitialOwnerPrincipalID *string `json:"initialOwnerPrincipalId,omitempty"`
	// InitialOwnerTenantID - Initial owner tenant Id.
	InitialOwnerTenantID *string `json:"initialOwnerTenantId,omitempty"`
}

// PutSubscriptionResponse subscription information (same as Microsoft.Resources/subscription).
type PutSubscriptionResponse struct {
	autorest.Response `json:"-"`
	// ID - Fully qualified ID for the subscription resource.
	ID *string `json:"id,omitempty"`
	// Name - Subscription ID.
	Name *string `json:"name,omitempty"`
	// Type - Resource type, Microsoft.Subscriptions.
	Type *string `json:"type,omitempty"`
	// Properties - Put subscription response properties.
	Properties *PutSubscriptionResponseProperties `json:"properties,omitempty"`
}

// PutSubscriptionResponseProperties put subscription creation result properties.
type PutSubscriptionResponseProperties struct {
	// DisplayName - The friendly name of the subscription.
	DisplayName *string `json:"displayName,omitempty"`
	// SkuID - The SKU ID of the Azure plan.
	SkuID *string `json:"skuId,omitempty"`
	// BillingScope - Corresponding billing scope.
	BillingScope *string `json:"billingScope,omitempty"`
	// InitialOwnerPrincipalID - Initial owner principal Id.
	InitialOwnerPrincipalID *string `json:"initialOwnerPrincipalId,omitempty"`
	// InitialOwnerTenantID - Initial owner tenant Id.
