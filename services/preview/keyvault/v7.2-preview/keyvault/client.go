...
// restoreBlobDetails - the Azure blob SAS token pointing to a folder where the previous successful full backup
// was stored
func (client BaseClient) SelectiveKeyRestoreOperationMethod(ctx context.Context, vaultBaseURL string, keyName string, restoreBlobDetails *SelectiveKeyRestoreOperationParameters) (result SelectiveKeyRestoreOperation, err error) {
	if tracing.IsEnabled() {
	...
	resp, err := client.SelectiveKeyRestoreOperationMethodSender(req)
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		err = autorest.NewErrorWithError(err, "keyvault.BaseClient", "SelectiveKeyRestoreOperationMethod", resp, "Failure sending request")
		return
	}

	result, err = client.SelectiveKeyRestoreOperationMethodResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "keyvault.BaseClient", "SelectiveKeyRestoreOperationMethod", resp, "Failure responding to request")
	}
...
// http.Response Body if it receives an error.
func (client BaseClient) SelectiveKeyRestoreOperationMethodSender(req *http.Request) (*http.Response, error) {
	return client.Send(req, autorest.DoRetryForStatusCodes(client.RetryAttempts, client.RetryDuration, autorest.StatusCodesForRetry...))
}
