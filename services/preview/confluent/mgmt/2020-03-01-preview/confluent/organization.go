...
// GetAgreementMarketplaceTerms get marketplace agreement in the subscription.
func (client OrganizationClient) GetAgreementMarketplaceTerms(ctx context.Context) (result AgreementTerms, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/OrganizationClient.GetAgreementMarketplaceTerms")
		defer func() {
			sc := -1
			if result.Response.Response != nil {
				sc = result.Response.Response.StatusCode
	...
	req, err := client.GetAgreementMarketplaceTermsPreparer(ctx)
	if err != nil {
		err = autorest.NewErrorWithError(err, "confluent.OrganizationClient", "GetAgreementMarketplaceTerms", nil, "Failure preparing request")
		return
	}

	resp, err := client.GetAgreementMarketplaceTermsSender(req)
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		err = autorest.NewErrorWithError(err, "confluent.OrganizationClient", "GetAgreementMarketplaceTerms", resp, "Failure sending request")
		return
	}

	result, err = client.GetAgreementMarketplaceTermsResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "confluent.OrganizationClient", "GetAgreementMarketplaceTerms", resp, "Failure responding to request")
	}

	return
}

// GetAgreementMarketplaceTermsPreparer prepares the GetAgreementMarketplaceTerms request.
func (client OrganizationClient) GetAgreementMarketplaceTermsPreparer(ctx context.Context) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"subscriptionId": autorest.Encode("path", client.SubscriptionID),
	}
...
// GetAgreementMarketplaceTermsSender sends the GetAgreementMarketplaceTerms request. The method will close the
// http.Response Body if it receives an error.
func (client OrganizationClient) GetAgreementMarketplaceTermsSender(req *http.Request) (*http.Response, error) {
	return client.Send(req, azure.DoRetryWithRegistration(client.Client))
}

// GetAgreementMarketplaceTermsResponder handles the response to the GetAgreementMarketplaceTerms request. The method always
// closes the http.Response Body.
func (client OrganizationClient) GetAgreementMarketplaceTermsResponder(resp *http.Response) (result AgreementTerms, err error) {
	err = autorest.Respond(
		resp,
		azure.WithErrorUnlessStatusCode(http.StatusOK),
		autorest.ByUnmarshallingJSON(&result),
