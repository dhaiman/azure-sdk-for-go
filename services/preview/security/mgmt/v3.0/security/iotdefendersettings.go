...
// DownloadOnPremiseActivation download on-premise sensors activation file
func (client IotDefenderSettingsClient) DownloadOnPremiseActivation(ctx context.Context) (result autorest.Response, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/IotDefenderSettingsClient.DownloadOnPremiseActivation")
		defer func() {
			sc := -1
			if result.Response != nil {
				sc = result.Response.StatusCode
	...
	if err := validation.Validate([]validation.Validation{
		{TargetValue: client.SubscriptionID,
			Constraints: []validation.Constraint{{Target: "client.SubscriptionID", Name: validation.Pattern, Rule: `^[0-9A-Fa-f]{8}-([0-9A-Fa-f]{4}-){3}[0-9A-Fa-f]{12}$`, Chain: nil}}}}); err != nil {
		return result, validation.NewError("security.IotDefenderSettingsClient", "DownloadOnPremiseActivation", err.Error())
	}

	req, err := client.DownloadOnPremiseActivationPreparer(ctx)
	if err != nil {
		err = autorest.NewErrorWithError(err, "security.IotDefenderSettingsClient", "DownloadOnPremiseActivation", nil, "Failure preparing request")
		return
	}

	resp, err := client.DownloadOnPremiseActivationSender(req)
	if err != nil {
		result.Response = resp
		err = autorest.NewErrorWithError(err, "security.IotDefenderSettingsClient", "DownloadOnPremiseActivation", resp, "Failure sending request")
		return
	}

	result, err = client.DownloadOnPremiseActivationResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "security.IotDefenderSettingsClient", "DownloadOnPremiseActivation", resp, "Failure responding to request")
	}

	return
}

// DownloadOnPremiseActivationPreparer prepares the DownloadOnPremiseActivation request.
func (client IotDefenderSettingsClient) DownloadOnPremiseActivationPreparer(ctx context.Context) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"subscriptionId": autorest.Encode("path", client.SubscriptionID),
	}
	...
	preparer := autorest.CreatePreparer(
		autorest.AsPost(),
		autorest.WithBaseURL(client.BaseURI),
		autorest.WithPathParameters("/subscriptions/{subscriptionId}/providers/Microsoft.Security/iotDefenderSettings/default/downloadOnPremiseActivation", pathParameters),
		autorest.WithQueryParameters(queryParameters))
	return preparer.Prepare((&http.Request{}).WithContext(ctx))
}

// DownloadOnPremiseActivationSender sends the DownloadOnPremiseActivation request. The method will close the
// http.Response Body if it receives an error.
func (client IotDefenderSettingsClient) DownloadOnPremiseActivationSender(req *http.Request) (*http.Response, error) {
	return client.Send(req, azure.DoRetryWithRegistration(client.Client))
}

// DownloadOnPremiseActivationResponder handles the response to the DownloadOnPremiseActivation request. The method always
// closes the http.Response Body.
func (client IotDefenderSettingsClient) DownloadOnPremiseActivationResponder(resp *http.Response) (result autorest.Response, err error) {
	err = autorest.Respond(
		resp,
