...
func (client IotSensorsClient) DownloadResetPassword(ctx context.Context, scope string, iotSensorName string, applianceID string) (result ReadCloser, err error) {
	if tracing.IsEnabled() {
	...
	req, err := client.DownloadResetPasswordPreparer(ctx, scope, iotSensorName, applianceID)
	if err != nil {
...
func (client IotSensorsClient) DownloadResetPasswordPreparer(ctx context.Context, scope string, iotSensorName string, applianceID string) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"iotSensorName": autorest.Encode("path", iotSensorName),
		"scope":         scope,
	}
