...
// RegionalServiceResource properties of a Regional Service Resource.
type RegionalServiceResource struct {
	// Location - The location of Regional Service Resource.
	Location *string `json:"location,omitempty"`
	// Status - The status of the regional service.
	Status *string `json:"status,omitempty"`
}
...
// ServiceGetResults an Azure Cosmos DB ServiceGetResults object.
type ServiceGetResults struct {
	autorest.Response `json:"-"`
	// ServiceResource - The properties of a Service Resource
	*ServiceResource `json:"properties,omitempty"`
}

// MarshalJSON is the custom marshaler for ServiceGetResults.
func (sgr ServiceGetResults) MarshalJSON() ([]byte, error) {
	objectMap := make(map[string]interface{})
	if sgr.ServiceResource != nil {
		objectMap["properties"] = sgr.ServiceResource
	}
	return json.Marshal(objectMap)
}

// UnmarshalJSON is the custom unmarshaler for ServiceGetResults struct.
func (sgr *ServiceGetResults) UnmarshalJSON(body []byte) error {
	var m map[string]*json.RawMessage
	err := json.Unmarshal(body, &m)
	if err != nil {
		return err
	...
	for k, v := range m {
		...
		case "properties":
			if v != nil {
				...
				err = json.Unmarshal(*v, &serviceResource)
				if err != nil {
					return err
				}
				sgr.ServiceResource = &serviceResource
			}
		}
	}
...
// ServiceResource an Azure CosmosDB Service Resource.
type ServiceResource struct {
	// ServiceType - Specifies the type of Service. Possible values include: 'Sqlx'
	ServiceType ServiceType `json:"serviceType,omitempty"`
	// Status - Status of the Azure CosmosDB Service Resource.
	Status *string `json:"status,omitempty"`
	// InstanceSize - Size of the CosmosDB Service Resource.
	InstanceSize *string `json:"instanceSize,omitempty"`
	// InstanceCount - Instance count of the Service Resource.
	InstanceCount *string `json:"instanceCount,omitempty"`
	// Locations - Map of a location name to Regional Service Resource.
	Locations *[]RegionalServiceResource `json:"locations,omitempty"`
}

// ServiceResourceCreateUpdateParameters parameters to create a Service Resource
type ServiceResourceCreateUpdateParameters struct {
	// ServiceResource - Properties to create and update an Azure Cosmos DB Service Resource.
	*ServiceResource `json:"properties,omitempty"`
}
...
func (srcup ServiceResourceCreateUpdateParameters) MarshalJSON() ([]byte, error) {
	objectMap := make(map[string]interface{})
	if srcup.ServiceResource != nil {
		objectMap["properties"] = srcup.ServiceResource
	}
	return json.Marshal(objectMap)
}
	...
	for k, v := range m {
		...
		case "properties":
			if v != nil {
				...
				srcup.ServiceResource = &serviceResource
			}
		}
	}
...
// SqlxCreateOrUpdateFuture an abstraction for monitoring and retrieving the results of a long-running
// operation.
type SqlxCreateOrUpdateFuture struct {
	azure.Future
}

// Result returns the result of the asynchronous operation.
// If the operation has not completed it will return an error.
func (future *SqlxCreateOrUpdateFuture) Result(client SqlxClient) (sgr ServiceGetResults, err error) {
	var done bool
	done, err = future.DoneWithContext(context.Background(), client)
	if err != nil {
		err = autorest.NewErrorWithError(err, "documentdb.SqlxCreateOrUpdateFuture", "Result", future.Response(), "Polling failure")
		return
	}
	if !done {
		err = azure.NewAsyncOpIncompleteError("documentdb.SqlxCreateOrUpdateFuture")
		return
	}
	sender := autorest.DecorateSender(client, autorest.DoRetryForStatusCodes(client.RetryAttempts, client.RetryDuration, autorest.StatusCodesForRetry...))
	if sgr.Response.Response, err = future.GetResult(sender); err == nil && sgr.Response.Response.StatusCode != http.StatusNoContent {
		sgr, err = client.CreateOrUpdateResponder(sgr.Response.Response)
		if err != nil {
			err = autorest.NewErrorWithError(err, "documentdb.SqlxCreateOrUpdateFuture", "Result", sgr.Response.Response, "Failure responding to request")
		}
	}
	return
}

// SqlxDeleteFuture an abstraction for monitoring and retrieving the results of a long-running operation.
type SqlxDeleteFuture struct {
	azure.Future
}

// Result returns the result of the asynchronous operation.
// If the operation has not completed it will return an error.
func (future *SqlxDeleteFuture) Result(client SqlxClient) (ar autorest.Response, err error) {
	var done bool
	done, err = future.DoneWithContext(context.Background(), client)
	if err != nil {
		err = autorest.NewErrorWithError(err, "documentdb.SqlxDeleteFuture", "Result", future.Response(), "Polling failure")
		return
	}
	if !done {
		err = azure.NewAsyncOpIncompleteError("documentdb.SqlxDeleteFuture")
		return
	}
	ar.Response = future.Response()
	return
