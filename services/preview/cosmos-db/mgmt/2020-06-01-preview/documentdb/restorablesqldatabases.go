...
// Get get the properties of a restorable SQL database.
// Parameters:
// location - cosmos DB region, with spaces between words and each word capitalized.
// instanceID - the instanceId GUID of a restorable database account.
// restorableSQLDatabaseID - the GUID of the restorable SQL database.
func (client RestorableSQLDatabasesClient) Get(ctx context.Context, location string, instanceID string, restorableSQLDatabaseID string) (result RestorableSQLDatabaseGetResult, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/RestorableSQLDatabasesClient.Get")
		defer func() {
			sc := -1
	...
	if err := validation.Validate([]validation.Validation{
		...
		return result, validation.NewError("documentdb.RestorableSQLDatabasesClient", "Get", err.Error())
	}

	req, err := client.GetPreparer(ctx, location, instanceID, restorableSQLDatabaseID)
	if err != nil {
		err = autorest.NewErrorWithError(err, "documentdb.RestorableSQLDatabasesClient", "Get", nil, "Failure preparing request")
		return
	}
	...
	if err != nil {
		...
		err = autorest.NewErrorWithError(err, "documentdb.RestorableSQLDatabasesClient", "Get", resp, "Failure sending request")
		return
	}

	result, err = client.GetResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "documentdb.RestorableSQLDatabasesClient", "Get", resp, "Failure responding to request")
	}
...
func (client RestorableSQLDatabasesClient) GetPreparer(ctx context.Context, location string, instanceID string, restorableSQLDatabaseID string) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"instanceId":              autorest.Encode("path", instanceID),
		"location":                autorest.Encode("path", location),
		"restorableSqlDatabaseId": autorest.Encode("path", restorableSQLDatabaseID),
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPathParameters("/subscriptions/{subscriptionId}/providers/Microsoft.DocumentDB/locations/{location}/restorableDatabaseAccounts/{instanceId}/restorableSqlDatabases/{restorableSqlDatabaseId}", pathParameters),
...
func (client RestorableSQLDatabasesClient) GetSender(req *http.Request) (*http.Response, error) {
	return client.Send(req, azure.DoRetryWithRegistration(client.Client))
}

// GetResponder handles the response to the Get request. The method always
// closes the http.Response Body.
func (client RestorableSQLDatabasesClient) GetResponder(resp *http.Response) (result RestorableSQLDatabaseGetResult, err error) {
