...
// DataConnectionsDataConnectionValidationAsyncFuture an abstraction for monitoring and retrieving the
// results of a long-running operation.
type DataConnectionsDataConnectionValidationAsyncFuture struct {
	azure.Future
}

// Result returns the result of the asynchronous operation.
// If the operation has not completed it will return an error.
func (future *DataConnectionsDataConnectionValidationAsyncFuture) Result(client DataConnectionsClient) (dcvlr DataConnectionValidationListResult, err error) {
	var done bool
	done, err = future.DoneWithContext(context.Background(), client)
	if err != nil {
		err = autorest.NewErrorWithError(err, "kusto.DataConnectionsDataConnectionValidationAsyncFuture", "Result", future.Response(), "Polling failure")
		return
	}
	if !done {
		err = azure.NewAsyncOpIncompleteError("kusto.DataConnectionsDataConnectionValidationAsyncFuture")
		return
	}
	sender := autorest.DecorateSender(client, autorest.DoRetryForStatusCodes(client.RetryAttempts, client.RetryDuration, autorest.StatusCodesForRetry...))
	if dcvlr.Response.Response, err = future.GetResult(sender); err == nil && dcvlr.Response.Response.StatusCode != http.StatusNoContent {
		dcvlr, err = client.DataConnectionValidationAsyncResponder(dcvlr.Response.Response)
		if err != nil {
			err = autorest.NewErrorWithError(err, "kusto.DataConnectionsDataConnectionValidationAsyncFuture", "Result", dcvlr.Response.Response, "Failure responding to request")
		}
	}
	return
}
