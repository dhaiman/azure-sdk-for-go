...
func (client TransfersClient) CancelPreparer(ctx context.Context, billingAccountName string, billingProfileName string, invoiceSectionName string, transferName string) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPathParameters("/providers/Microsoft.Billing/billingAccounts/{billingAccountName}/billingProfiles/{billingProfileName}/invoiceSections/{invoiceSectionName}/transfers/{transferName}/cancel", pathParameters))
...
func (client TransfersClient) ChangeInvoiceSection(ctx context.Context, billingAccountName string, parameters TransferBillingSubscriptionRequestProperties) (result autorest.Response, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/TransfersClient.ChangeInvoiceSection")
		defer func() {
			sc := -1
	...
	if err := validation.Validate([]validation.Validation{
		...
		return result, validation.NewError("billing.TransfersClient", "ChangeInvoiceSection", err.Error())
	}

	req, err := client.ChangeInvoiceSectionPreparer(ctx, billingAccountName, parameters)
	if err != nil {
		err = autorest.NewErrorWithError(err, "billing.TransfersClient", "ChangeInvoiceSection", nil, "Failure preparing request")
		return
	}

	resp, err := client.ChangeInvoiceSectionSender(req)
	if err != nil {
		result.Response = resp
		err = autorest.NewErrorWithError(err, "billing.TransfersClient", "ChangeInvoiceSection", resp, "Failure sending request")
		return
	}

	result, err = client.ChangeInvoiceSectionResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "billing.TransfersClient", "ChangeInvoiceSection", resp, "Failure responding to request")
	}
...
func (client TransfersClient) ChangeInvoiceSectionPreparer(ctx context.Context, billingAccountName string, parameters TransferBillingSubscriptionRequestProperties) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"billingAccountName": autorest.Encode("path", billingAccountName),
...
// http.Response Body if it receives an error.
func (client TransfersClient) ChangeInvoiceSectionSender(req *http.Request) (*http.Response, error) {
	return client.Send(req, autorest.DoRetryForStatusCodes(client.RetryAttempts, client.RetryDuration, autorest.StatusCodesForRetry...))
}

// ChangeInvoiceSectionResponder handles the response to the ChangeInvoiceSection request. The method always
// closes the http.Response Body.
func (client TransfersClient) ChangeInvoiceSectionResponder(resp *http.Response) (result autorest.Response, err error) {
	err = autorest.Respond(
		resp,
