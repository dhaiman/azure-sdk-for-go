...
const (
	...
	DefaultBaseURI = "https://api.bing.microsoft.com/v7.0"
)
...
// [cc](https://docs.microsoft.com/en-us/bing/bing-autosuggest/overview) query parameter. To determine the
// market to return results for, Bing uses the first supported language it finds from the list and combines it
// with the cc parameter value. If the list does not include a supported language, Bing finds the closest
// language and market that supports the request or it uses an aggregated or default market for the results. To
// determine the market that Bing used, see the BingAPIs-Market header. Use this header and the cc query
...
// [setLang](https://docs.microsoft.com/en-us/bing/bing-autosuggest/overview) query parameters. A user
// interface string is a string that's used as a label in a user interface. There are few user interface
// strings in the JSON response objects. Any links to Bing.com properties in the response objects apply the
// specified language.
